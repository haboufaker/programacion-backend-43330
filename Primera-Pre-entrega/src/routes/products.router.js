...
productsRouter.post('/', async (req, res) => {
    try {
		...
		let newProduct = await productManager.addProduct(product.title, product.description, product.price, product.code, product.stock, product.thumbnail)
        if (newProduct === 400) {
...
productsRouter.put('/:pid', async (req, res) => {
    try {
		let id = Math.round(req.params.pid)
		let product = req.body;
        let existingProduct = await productManager.updateProduct(id, product)
...
productsRouter.delete('/:pid', async (req, res) => {
    try {
		let id = Math.round(req.params.pid)
        let existingProduct = await productManager.deleteProduct(id)
